...
void flash_erase_sector(flash_t *obj, u32 address)
{
	...
	volatile float ms_passed = 0;

    
	flash_write_lock();

    
	/*  Store gtimer timestamp before write */
	...
	ms_passed = (((float)(tick_passed * 1000))/32768);
	ms_passed += missing_tick;
	missing_tick = ms_passed - (float)((int)(ms_passed*1000)/1000);
	
	vTaskStepTick((int)(ms_passed*1000)/1000); /*  update kernel tick */
