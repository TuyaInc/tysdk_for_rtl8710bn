...
typedef struct led_cntl {
    ...
    TY_GPIO_PORT_E port;
    LED_LT_E type;
    USHORT flash_cycle; // ms
    USHORT expire_time;
    USHORT flh_sum_time; // if(flh_sum_time == 0xffff) then flash forever;
}LED_CNTL_S;
...
*************************function define********************
***********************************************************/
STATIC VOID __led_timer_cb(UINT timerID,PVOID pTimerArg);
...
OPERATE_RET tuya_create_led_handle(IN CONST TY_GPIO_PORT_E port,IN CONST BOOL_T high,OUT LED_HANDLE *handle)
{
    ...
    if(NULL == mutex) {
        ...
        if(OPRT_OK != op_ret) {
            PR_ERR("CreateMutexAndInit err:%d",op_ret);
            return op_ret;
        }
    }
